...
class FormationService {
  ...
  initialize(blockData: BlockData): void {
    ...
    // Store the block height for logging purposes
    const blockHeight = blockData.height;
    console.log(`Formation service initialized with block ${blockHeight}`);
  }
   ...
   */
  getFormationsForRole(role: Role): FormationPattern[] {
    if (!this.initialized || !this.rngSystem) {
      throw new Error('Formation service not initialized');
    }

    // TODO: Implement proper formation retrieval based on role
    console.log(`Getting formations for role: ${role}`);
    return [];
  }
